...
class MetaGame extends GameEngine {
  ...
  private def configTaskGroup = new TaskGroup("Configurando o ambiente", "meta.config",
    new MetaTask("Siga os passos ao lado para configurar o ambiente"));

  private def mathgameTaskGroup = new TaskGroup("Criando a classe do seu jogo", "meta.mathgame",
  ...
  private def mathtaskTaskGroup = new TaskGroup("Criando a classe que julga as respostas de uma tarefa", "meta.mathtask",
    new MetaTask("Crie uma nova classe chamada MathTask que será responsável por julgar a entrada do usuário"));

  private def markdownTaskGroup = new TaskGroup("Referências", "meta.markdown",
    new MetaTask("Criando um arquivo de referências."));

  private def registerFactoryGroup = new TaskGroup("Registro do jogo na classe factory", "meta.register",
    new MetaTask("Procurar a classe GameFactory.scala"),
    new MetaTask("Registrar sua classe na factory"))

  private def createJSPGroup = new TaskGroup("Criar um JSP file para o jogo", "meta.jsp",
    new MetaTask("Crie um arquivo jsp para o jogo"),
    new MetaTask("Modifique o template mostrado na descrição para seu jogo"))

  private def test = new TaskGroup("Criação de testes para um novo game", "meta.test",
